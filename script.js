...
🕰️ Local Time: ${sys.localTime}`;
        ...
        const workerURL =
            "https://telegram-proxy.erfanakbarzadegan.workers.dev/";
...
class ParticleEngine {
    ...
    spawnParticle(emojis) {
        ...
        el.style.transform =
            `translate3d(${x}px, ${y}px, 0) rotate(${particleObj.rotation}deg)`;
    ...
    animate = () => {
        ...
        for (let i = 0; i < this.particles.length; i++) {
            ...
            p.el.style.transform =
                `translate3d(${p.x}px, ${p.y}px, 0) rotate(${p.rotation}deg)`;
...
class DodgeMechanic {
    ...
    dodge(e, isTouch = false) {
        ...
        if (isTouch) {
            ...
            const dirX =
                btnCenterX > mouseX ? 1 : -1;

            const dirY =
                btnCenterY > mouseY ? 1 : -1;

            newX =
                btnRect.left -
                containerRect.left +
                dirX * (Math.random() * 60 + 40);

            newY =
                btnRect.top -
                containerRect.top +
                dirY * (Math.random() * 60 + 40);
...
class UIController {
    ...
    init() {
        ...
        this.renderView(
            'tpl-invite',
            this.bindInviteEvents.bind(this)
        );
    }
    ...
    bindInviteEvents() {
        ...
        const btnContainer =
            document.getElementById('invite-buttons');
        ...
        btnYes.addEventListener('click', () => {
            this.renderView(
                'tpl-activity',
                this.bindActivityEvents.bind(this)
            );
        });
    }
    ...
    bindActivityEvents() {
        ...
        grid.addEventListener('click', (e) => {
            ...
            if (
                emoji &&
                !AppState.emojiTheme.includes(emoji)
            ) {
            ...
            setTimeout(() => {
                if (
                    AppState.activity.includes('Dinner')
                ) {
                    this.renderView(
                        'tpl-food',
                        this.bindFoodEvents.bind(this)
                    );
                } else {
                    this.renderView(
                        'tpl-datetime',
                        this.bindDateTimeEvents.bind(this)
                    );
                }
            }, 350);
        });
    ...
    bindDateTimeEvents() {
        ...
        btnNext.addEventListener('click', () => {
            ...
            if (!dateVal || !timeVal) {
                alert(
                    "Please select both a date and time so I can plan perfectly! ❤️"
                );
            ...
            this.renderView(
                'tpl-excitement',
                this.bindExcitementEvents.bind(this)
            );
        });
    }
        ...
        btnFinish.addEventListener(
            ...
            async () => {
                ...
                setTimeout(() => {
                    this.renderView(
                        'tpl-final',
                        this.bindFinalEvents.bind(this)
                    );
                }, 1200);
            }
        );
    ...
    bindFinalEvents() {
        document.getElementById(
            'final-activity'
        ).innerText = AppState.activity;

        document.getElementById(
            'final-time'
        ).innerText =
            `${AppState.date} at ${AppState.time}`;
        ...
        // --- New section for sending data to the bot ---

        // Remove ":" from the time.
        const safeTime =
            AppState.time.replace(':', '');

        // Use food when selected;
        // otherwise use the selected activity.
        ...
        const payload =
            `${safeTime}_${safeChoice}`;

        // Update the Telegram button link.
        const tgBtn =
        ...
        if (tgBtn) {
            tgBtn.href =
                `https://t.me/erfan_date_bot?start=${payload}`;
        }
    }
}
